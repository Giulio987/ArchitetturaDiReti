...
int main(int argc, char** argv)
{
        ...
        for (;;) {
                ...
                if ((pid = fork())<0){
                        ...
                        for (;;) {
                                int fd, cc, pid_n1, pid_n2, pipe_n1n2[2], status;
                                char template[256], message[1024], *categoria;
                                off_t off, file_size;
                                char *csexp_strings[1];
                                size_t csexp_strings_len;

                                /* Leggo messaggio richiesta da Client */
                                csexp_strings_len = 1;
                                cc = simple_csexp_read_message(&rxb, ns, csexp_strings, &csexp_strings_len);
                                if (cc < 0) {
                                ...
                                categoria = csexp_strings[0];

#ifdef USE_LIBUNISTRING
                                /* Verifico che il messaggio sia UTF-8 valido */
                                if (u8_check(categoria, strlen(categoria)) != NULL) {
                                ...
                                snprintf(message, sizeof(message), "(%ld:", file_size);
                                ...
                                off = 0;
                                cc = sendfile(ns, fd, &off, file_size);
                                if (cc < 0) {
                                        perror("sendfile");
                                        exit(EXIT_FAILURE);
                                }
