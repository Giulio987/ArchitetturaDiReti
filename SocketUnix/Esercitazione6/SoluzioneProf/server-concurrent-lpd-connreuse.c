...
int main(int argc, char **argv)
{
		...
		/* Creo un processo figlio per gestire la richiesta */
			...
			if (autorizza(username, password) == 1) {
				write_all(ns, ack, sizeof(ack)) {
					perror("write");
					exit(EXIT_FAILURE);
				}
			} else {
				write_all(ns, nack, sizeof(nack)) {
					perror("write");
					exit(EXIT_FAILURE);
				}
				close(ns);
			...
			for (;;) {
				...
				while ((nread = read(pipe_n3f, response + read_so_far, 
